...
/**
 * HTMLを生成する関数（generate-all-regions-full.jsのテンプレートを使用）
 */
...
async function main() {
  ...
  console.log(`${colors.cyan}  全地域HTMLページ生成スクリプト${colors.reset}`);
  console.log(`${colors.cyan}==================================================${colors.reset}\n`);
  ...
  console.log(`${colors.yellow}合計: ${allRegions.length}件の地域を処理します${colors.reset}\n`);
  ...
  console.log(`${colors.yellow}重複除去後: ${uniqueRegions.length}件の地域を処理します${colors.reset}\n`);
  ...
  for (let i = 0; i < uniqueRegions.length; i++) {
    const result = await processRegion(uniqueRegions[i], i, uniqueRegions.length, convenienceLogos);
    results.push(result);

    // APIレート制限を避けるため少し待機
    if (i < uniqueRegions.length - 1) {
  ...
  console.log(`${colors.green}✓ 処理完了!${colors.reset}\n`);
  ...
  // 失敗した地域を表示
  if (failCount > 0) {
    console.log(`\n${colors.red}失敗した地域:${colors.reset}`);
  ...
  const stats = {
    ...
    results: results.map(r => ({
      ...
      hotspringCount: r.hotspringCount,
      touristSpotCount: r.touristSpotCount
    }))
  };

  fs.writeFileSync(
    path.join(OUTPUT_DIR, '_generation_stats.json'),
    JSON.stringify(stats, null, 2),
    'utf8'
  );

  console.log(`\n${colors.cyan}統計情報: ${OUTPUT_DIR}/_generation_stats.json${colors.reset}`);
  console.log(`${colors.cyan}==================================================${colors.reset}\n`);
}
