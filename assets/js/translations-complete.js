// Complete translations for index.html - All 10 languages
const completeTranslations = {
    ja: {
        // Meta tags
        meta: {
            title: "TrailFusion AI - キャンピングカーDIYとAIアプリケーション開発",
            description: "キャンピングカーDIY制作からAIアプリケーション開発まで。ハイエースを自作キャンピングカーに改造する実践ガイドと革新的なAIアプリを紹介。"
        },
        
        // Navigation
        nav: {
            home: "ホーム",
            apps: "アプリ紹介",
            camping: "キャンピングカー製作",
            contact: "お問い合わせ"
        },
        
        // Hero section
        hero: {
            line1: "自由な旅と",
            line2: "革新的なAI体験",
            line3: "を創造する",
            description: "キャンピングカーDIYとAIアプリケーションで、あなたの夢を現実に",
            campingButton: "キャンピングカー製作",
            appsButton: "アプリ紹介",
            scrollText: "詳細を見る"
        },
        
        // Services section
        services: {
            title: "TrailFusion AI のサービス",
            subtitle: "DIYの専門知識と最先端のAI技術を組み合わせ、ユーザーの夢の実現をサポートします",
            camping: {
                title: "ハイエースDIYキャンピングガイド",
                description: "初心者でもプロレベルのキャンピングカーを自作できるように、包括的なガイドを提供しています。特にハイエースの改造に焦点を当てており、8ナンバー登録まで完全にサポートします。",
                features: {
                    f1: "詳細な製作手順書",
                    f2: "必要な材料・工具のリスト",
                    f3: "構造要件のチェックリスト",
                    f4: "完成例のギャラリー"
                },
                cta: "キャンピングガイドを見る"
            },
            ai: {
                title: "革新的なAIアプリケーション",
                description: "日常生活や学習を豊かにするための、AIを活用した6つのアプリケーションを開発・提供しています。最新のAI技術を駆使し、ユーザーの生活をより便利で豊かにします。",
                features: {
                    f1: "GP Teachers: AI英語学習アプリ",
                    f2: "VoiceLink AI通訳: リアルタイム通訳アプリ",
                    f3: "Word Blaster: 英単語学習ゲーム",
                    f4: "その他、ディベート練習や作業タイマーなど"
                },
                cta: "アプリを見る"
            }
        },
        
        // About section
        about: {
            title: "代表者紹介",
            motto: "人生は、いつだって変えられる",
            subtitle: "やりたい事は老後でなく今やる決意",
            story: {
                p1: "日本が大好きでした。風情ある街並み、心と体を癒す温泉、土地ごとに味わえる美食、そして温かい人々。いつか、この愛する日本を隅々まで駆け巡るのが私の夢でした。",
                p2: "しかし、現実は多忙なサラリーマン。まとまった休みなど夢のまた夢で、「日本一周は老後の楽しみ」と、自分に言い聞かせ、諦めかけていました。",
                p3: "そんな私の人生が、180度変わる転機が訪れます。新型コロナウイルスへの感染でした。",
                p4: "たった一人、部屋に隔離された1週間。死の恐怖と隣り合わせの状況で、私の頭をよぎったのはたった一つの想いでした。「やりたいと思ったことは、今すぐやらなければ後悔する。人生は、あまりにも短い」",
                p5: "部屋から出られるようになったその日、私の第二の人生が始まりました。隔離されていたわずか1週間で、私はキャンピングカーを自作するための全ての計画を立て終えていたのです。",
                p6: "ベース車両の選定、4人家族が眠れるスライドベッドの設計、仕事もできるデスクの配置、法規をクリアするための8ナンバー登録の要件洗い出し、サブバッテリーシステムの構築、そして内装に使う木材の調達先まで。頭の中の設計図は、すっかり完成していました。",
                p7: "すぐさま、2008年式のハイエース スーパーロング（救急車と同サイズ！）を170万円で即決購入。そこから2ヶ月間、平日の仕事終わり、週末、リモートワークの休憩時間など、使える時間はすべてDIYに捧げました。",
                p8: "そして、目標としていた夏休み。家族との初めての車中泊旅行を前に、世界に一台だけのオリジナルキャンピングカーは、ついに完成したのです。",
                p9: "その旅は、想像を絶するほど感動的なものでした。満室のホテルを横目に、青森ねぶた祭や長岡花火大会の熱狂を最前線で体感する。夏の北海道では、高騰する宿代を気にすることなく10日間も大自然を満喫し、子供たちの満面の笑みに包まれる。",
                p10: "この経験を通じて、私は確信しました。「一台のクルマが、これほどまでに人生を豊かにしてくれるのか」と。",
                p11: "そして、この自由な旅は、私に次なる、そして生涯を懸けた夢を与えてくれました。それは、このキャンピングカーを「世界を旅する開発拠点」とし、場所に縛られることなく、世界中の文化や人々と触れ合いながら、新しい価値を創造することです。",
                p12: "具体的には、AI技術を活用した革新的なアプリケーションを開発し、Appleプラットフォームを通じて世界中のユーザーに届けること。旅先で得たインスピレーションをリアルタイムで開発に注ぎ込み、人々の生活をより便利で、より豊かにする。そんな貢献の形を、私は見つけました。",
                p13: "サラリーマンとして諦めかけた夢。しかし、一度きりの人生、もう後悔はしたくない。世界中を旅し、価値を創造し、出会いを楽しみ尽くす。そしていつか、人生の最期を迎えたときに「最高の人生だった」と心から笑いたい。「TrailfusionAI」は、その誓いの名です。"
            },
            callToAction: "さあ、あなたも私たちと一緒に、後悔しない人生という名の冒険へ出発しませんか？",
            followMe: "Follow Me",
            interests: "趣味・興味",
            tags: ["剣道", "キャンピングカー", "DIY制作", "日本一周", "グルメ", "温泉", "アプリ開発", "マラソン", "山登り"]
        },
        
        // Contact section
        contact: {
            title: "お問い合わせ",
            subtitle: "ご質問やご相談がありましたら、お気軽にお問い合わせください。",
            form: {
                name: "お名前",
                namePlaceholder: "山田 太郎",
                email: "メールアドレス",
                emailPlaceholder: "example@mail.com",
                subject: "件名",
                subjectPlaceholder: "お問い合わせ内容の件名",
                message: "メッセージ",
                messagePlaceholder: "ご質問やご相談内容をご記入ください",
                charCount: "文字",
                submit: "送信する"
            },
            info: {
                title: "お問い合わせ先",
                email: "メールアドレス",
                youtube: "TrailFusion AI チャンネル"
            },
            messages: {
                success: "お問い合わせを受け付けました。ありがとうございます。",
                error: "エラーが発生しました。もう一度お試しください。"
            }
        },
        
        // Footer
        footer: {
            description: "革新的なAIアプリケーションとDIYキャンピングカー製作ガイドで、あなたの可能性を広げます。",
            camping: {
                title: "キャンピングカー",
                items: {
                    videos: "DIY 製作の全工程を動画で公開",
                    gallery: "完成例ギャラリー",
                    guide: "DIY キャンピングカー製作ガイド",
                    requirements: "キャンピングカー構造要件"
                }
            },
            apps: "AIアプリ",
            contact: "お問い合わせ"
        }
    },
    
    en: {
        // Meta tags
        meta: {
            title: "TrailFusion AI - Camper DIY & AI Application Development",
            description: "From camper DIY creation to AI application development. Practical guide to converting a HiAce into a homemade camper and introducing innovative AI apps."
        },
        
        // Navigation
        nav: {
            home: "Home",
            apps: "Apps",
            camping: "Camper Manufacturing",
            contact: "Contact"
        },
        
        // Hero section
        hero: {
            line1: "Free Travel and",
            line2: "Innovative AI Experience",
            line3: "Creating",
            description: "Make your dreams come true with camper DIY and AI applications",
            campingButton: "Camper Manufacturing",
            appsButton: "Apps",
            scrollText: "See Details"
        },
        
        // Services section
        services: {
            title: "TrailFusion AI Services",
            subtitle: "Combining DIY expertise with cutting-edge AI technology to support users in realizing their dreams",
            camping: {
                title: "HiAce DIY Camping Guide",
                description: "We provide comprehensive guides so that even beginners can build professional-level campers. We focus particularly on HiAce modifications and provide complete support up to 8-number registration.",
                features: {
                    f1: "Detailed construction manual",
                    f2: "List of required materials and tools",
                    f3: "Structural requirements checklist",
                    f4: "Gallery of completed examples"
                },
                cta: "View Camping Guide"
            },
            ai: {
                title: "Innovative AI Applications",
                description: "We develop and provide six AI-powered applications to enrich daily life and learning. Using the latest AI technology, we make users' lives more convenient and enriched.",
                features: {
                    f1: "GP Teachers: AI English Learning App",
                    f2: "VoiceLink AI Interpreter: Real-time Translation App",
                    f3: "Word Blaster: English Vocabulary Learning Game",
                    f4: "Others include debate practice and work timers"
                },
                cta: "View Apps"
            }
        },
        
        // About section
        about: {
            title: "Representative Introduction",
            motto: "Life can always be changed",
            subtitle: "The determination to do what you want now, not in old age",
            story: {
                p1: "I loved Japan. The atmospheric streetscapes, hot springs that heal mind and body, local cuisine, and warm people. Someday, traveling throughout this beloved Japan was my dream.",
                p2: "However, the reality was that of a busy salaryman. Extended vacations were a pipe dream, and I told myself that 'touring Japan would be a pleasure for retirement' and was almost giving up.",
                p3: "Then came a turning point that would change my life 180 degrees. It was my infection with the novel coronavirus.",
                p4: "Alone, isolated in a room for a week. In a situation facing the fear of death, only one thought crossed my mind: 'If I want to do something, I must do it now or I'll regret it. Life is far too short.'",
                p5: "The day I could leave my room, my second life began. In just one week of isolation, I had finished planning everything needed to build my own camper.",
                p6: "Selecting the base vehicle, designing a slide bed for a family of four, arranging a desk for work, identifying requirements for 8-number registration to meet regulations, building a sub-battery system, and even sourcing wood for the interior. The blueprint in my head was completely finished.",
                p7: "I immediately bought a 2008 HiAce Super Long (same size as an ambulance!) for 1.7 million yen. From there, for two months, I devoted all available time to DIY - after work on weekdays, weekends, and breaks during remote work.",
                p8: "And then came the summer vacation I had been aiming for. Before the first car camping trip with my family, the one-of-a-kind original camper was finally completed.",
                p9: "That trip was unimaginably moving. Passing by fully booked hotels, experiencing the excitement of Aomori Nebuta Festival and Nagaoka Fireworks Festival on the front lines. In summer Hokkaido, we enjoyed nature for 10 days without worrying about skyrocketing accommodation costs, surrounded by our children's beaming smiles.",
                p10: "Through this experience, I became convinced: 'Can one vehicle enrich life to this extent?'",
                p11: "And this free journey gave me the next dream I would stake my life on. That is, to make this camper a 'development base for traveling the world,' creating new value while interacting with cultures and people around the world, without being bound by location.",
                p12: "Specifically, developing innovative applications using AI technology and delivering them to users worldwide through the Apple platform. Pouring inspiration gained from travels into development in real-time, making people's lives more convenient and enriched. I found this form of contribution.",
                p13: "Dreams I had almost given up on as a salaryman. But in this one life, I don't want to have any more regrets. Travel the world, create value, and fully enjoy encounters. And someday, when I face the end of my life, I want to smile from my heart saying 'It was the best life.' 'TrailfusionAI' is the name of that oath."
            },
            callToAction: "Now, won't you join us on an adventure called a life without regrets?",
            followMe: "Follow Me",
            interests: "Hobbies & Interests",
            tags: ["Kendo", "Camper", "DIY Production", "Japan Tour", "Gourmet", "Hot Springs", "App Development", "Marathon", "Mountain Climbing"],
            
            // Additional story paragraphs that were found in the HTML
            additionalStory: {
                p11: "And this free journey gave me the next dream I would stake my life on. That is, to make this camper a 'development base for traveling the world,' creating new value while interacting with cultures and people around the world, without being bound by location.",
                p12: "Specifically, developing innovative applications using AI technology and delivering them to users worldwide through the Apple platform. Pouring inspiration gained from travels into development in real-time, making people's lives more convenient and enriched. I found this form of contribution.",
                p13: "The flagship to realize this grand vision is 'Trail❌fusion❌AI'.",
                p14: "Trail (Road): The physical 'road' of traveling the world with a camper, and the life 'road' leading to a way of life no one has ever walked.",
                p15: "Fusion: The 'fusion' of real travel experiences with digital creation of AI app development. And 'fusing' the thoughts of users around the world with ours.",
                p16: "AI: The core of our business that opens up our future and creates infinite possibilities.",
                p17: "Dreams I had almost given up on as a salaryman. But in this one life, I don't want to have any more regrets. Travel the world, create value, and fully enjoy encounters. And someday, when I face the end of my life, I want to smile from my heart saying 'It was the best life.' 'TrailfusionAI' is the name of that oath.",
                finalCta: "Now, won't you join us on an adventure called a life without regrets?"
            }
        },
        
        // Mobile navigation
        mobileNav: {
            home: "Home",
            camping: "Camper Manufacturing",
            apps: "Apps",
            contact: "Contact"
        },
        
        // Contact section
        contact: {
            title: "Contact",
            subtitle: "If you have any questions or consultations, please feel free to contact us.",
            form: {
                name: "Name",
                namePlaceholder: "John Doe",
                email: "Email Address",
                emailPlaceholder: "example@mail.com",
                subject: "Subject",
                subjectPlaceholder: "Subject of inquiry",
                message: "Message",
                messagePlaceholder: "Please write your questions or consultation content",
                charCount: "characters",
                submit: "Send"
            },
            info: {
                title: "Contact Information",
                email: "Email Address",
                youtube: "TrailFusion AI Channel"
            },
            messages: {
                success: "We have received your inquiry. Thank you.",
                error: "An error occurred. Please try again.",
                sending: "Sending...",
                charCount: "characters"
            }
        },
        
        // Form messages and labels
        form: {
            sending: "Sending...",
            success: "We have received your inquiry. Thank you.",
            error: "An error occurred. Please try again.",
            charCount: "characters"
        },
        
        // Footer
        footer: {
            description: "Expand your possibilities with innovative AI applications and DIY camper construction guides.",
            camping: {
                title: "Camper",
                items: {
                    videos: "Complete DIY production process published in videos",
                    gallery: "Gallery of completed examples",
                    guide: "DIY Camper Construction Guide",
                    requirements: "Camper Structural Requirements"
                }
            },
            apps: "AI Apps",
            contact: "Contact",
            info: {
                title: "Contact Information",
                email: "Email Address",
                youtube: "YouTube",
                tiktok: "TikTok",
                twitter: "X (formerly Twitter)"
            }
        }
    },
    
    // Russian translations (Complete version)
    ru: {
        meta: { title: "TrailFusion AI - DIY Дом на колёсах и Разработка AI Приложений", description: "От создания DIY дома на колёсах до разработки AI приложений. Практическое руководство по переоборудованию HiAce в самодельный дом на колёсах и представление инновационных AI приложений." },
        nav: { home: "Главная", apps: "AI Приложения", camping: "Изготовление Дома на колёсах", contact: "Контакты" },
        hero: { line1: "Свободные путешествия и", line2: "Инновационный AI опыт", line3: "Создание", description: "Воплотите свои мечты в реальность с DIY домом на колёсах и AI приложениями", campingButton: "Изготовление Дома на колёсах", appsButton: "AI Приложения", scrollText: "Подробнее" },
        services: { title: "Услуги TrailFusion AI", subtitle: "Сочетая экспертизу DIY с передовыми AI технологиями для поддержки пользователей в реализации их мечт", camping: { title: "Руководство по DIY кемпингу HiAce", description: "Мы предоставляем комплексные руководства, чтобы даже новички могли построить дома на колёсах профессионального уровня. Мы особенно фокусируемся на модификациях HiAce и обеспечиваем полную поддержку вплоть до регистрации номера 8.", features: { f1: "Подробное руководство по строительству", f2: "Список необходимых материалов и инструментов", f3: "Контрольный список структурных требований", f4: "Галерея завершенных примеров" }, cta: "Посмотреть руководство по кемпингу" }, ai: { title: "Инновационные AI приложения", description: "Мы разрабатываем и предоставляем 6 приложений на базе AI для обогащения повседневной жизни и обучения. Используя новейшие AI технологии, мы делаем жизнь пользователей более удобной и богатой.", features: { f1: "GP Teachers: AI приложение для изучения английского", f2: "VoiceLink AI переводчик: Приложение для перевода в реальном времени", f3: "Word Blaster: Игра для изучения английской лексики", f4: "Другие включают практику дебатов и рабочие таймеры" }, cta: "Посмотреть приложения" } },
        about: { title: "Представление представителя", motto: "Жизнь всегда можно изменить", subtitle: "Решимость делать то, что хочешь сейчас, а не в старости", story: { p1: "Я любил Японию. Атмосферные городские пейзажи, горячие источники, исцеляющие разум и тело, местная кухня и теплые люди. Когда-нибудь путешествовать по этой любимой Японии было моей мечтой.", p2: "Однако реальность была такова - занятый офисный работник. Продолжительные отпуска были несбыточной мечтой, и я говорил себе, что 'путешествие по Японии будет удовольствием для пенсии' и почти сдавался.", p3: "Затем пришла поворотная точка, которая изменила мою жизнь на 180 градусов. Это была моя инфекция новым коронавирусом.", p4: "Один, изолированный в комнате на неделю. В ситуации, столкнувшись со страхом смерти, только одна мысль пронеслась в моей голове: 'Если я хочу что-то сделать, я должен сделать это сейчас, иначе пожалею. Жизнь слишком коротка.'", p5: "В день, когда я смог покинуть свою комнату, началась моя вторая жизнь. Всего за одну неделю изоляции я закончил планировать всё необходимое для строительства своего собственного дома на колёсах.", p6: "Выбор базового автомобиля, проектирование раздвижной кровати для семьи из четырех человек, размещение стола для работы, определение требований для регистрации номера 8 для соблюдения правил, построение системы дополнительной батареи и даже поставка древесины для интерьера. План в моей голове был полностью готов.", p7: "Я немедленно купил HiAce Super Long 2008 года (того же размера, что и скорая помощь!) за 1,7 миллиона йен. С того момента, в течение двух месяцев, я посвятил всё доступное время DIY - после работы в будние дни, выходные и перерывы во время удаленной работы.", p8: "И затем наступили летние каникулы, к которым я стремился. Перед первым походом с семьей уникальный в мире оригинальный дом на колёсах наконец был завершен.", p9: "Это путешествие было невероятно трогательным. Проезжая мимо переполненных отелей, переживая волнение фестиваля Аомори Небута и фестиваля фейерверков Нагаока на переднем крае. Летом в Хоккайдо мы наслаждались природой в течение 10 дней, не беспокоясь о растущих расходах на проживание, окруженные сияющими улыбками наших детей.", p10: "Через этот опыт я убедился: 'Может ли одно транспортное средство обогатить жизнь до такой степени?'" }, additionalStory: { p11: "И это свободное путешествие дало мне следующую мечту, и ту, на которую я поставлю свою жизнь. Это сделать этот дом на колёсах 'базой разработки для путешествий по миру', создавая новые ценности, взаимодействуя с культурами и людьми по всему миру, не будучи привязанным к местоположению.", p12: "Конкретно, разрабатывать инновационные приложения, используя AI технологии, и доставлять их пользователям по всему миру через платформу Apple. Вливая вдохновение, полученное от путешествий, в разработку в реальном времени, делая жизнь людей более удобной и богатой. Я нашел эту форму вклада.", p13: "Флаг для реализации этого грандиозного видения - 'Trail❌fusion❌AI'.", p14: "Trail (Тропа): Физическая 'тропа' путешествия по миру с домом на колёсах, и жизненная 'тропа', ведущая к образу жизни, по которому никто никогда не ходил.", p15: "Fusion (Слияние): 'Слияние' реального опыта путешествий с цифровым творчеством разработки AI приложений. И 'слияние' мыслей пользователей по всему миру с нашими.", p16: "AI: Ядро нашего бизнеса, которое открывает наше будущее и создает бесконечные возможности.", p17: "Мечты, которые я почти оставил как офисный работник. Но в этой единственной жизни я больше не хочу сожалений. Путешествовать по миру, создавать ценность и полностью наслаждаться встречами. И когда-нибудь, когда я столкнусь с концом своей жизни, я хочу улыбнуться от всего сердца, говоря 'Это была лучшая жизнь'. 'TrailfusionAI' - это имя этой клятвы.", finalCta: "Теперь, не присоединитесь ли вы к нам в приключении под названием жизнь без сожалений?" }, callToAction: "Теперь, не присоединитесь ли вы к нам в приключении под названием жизнь без сожалений?", followMe: "Подписывайтесь", interests: "Хобби и интересы", tags: ["Кендо", "Дом на колёсах", "DIY производство", "Тур по Японии", "Гурман", "Горячие источники", "Разработка приложений", "Марафон", "Альпинизм"] },
        mobileNav: { home: "Главная", camping: "Изготовление Дома на колёсах", apps: "AI Приложения", contact: "Контакты" },
        contact: { title: "Контакты", subtitle: "Если у вас есть вопросы или консультации, пожалуйста, свяжитесь с нами.", form: { name: "Имя", namePlaceholder: "Иван Иванов", email: "Адрес электронной почты", emailPlaceholder: "example@mail.com", subject: "Тема", subjectPlaceholder: "Тема запроса", message: "Сообщение", messagePlaceholder: "Пожалуйста, напишите ваши вопросы или содержание консультации", charCount: "символов", submit: "Отправить" }, info: { title: "Контактная информация", email: "Адрес электронной почты", youtube: "Канал TrailFusion AI" }, messages: { success: "Мы получили ваш запрос. Спасибо.", error: "Произошла ошибка. Пожалуйста, попробуйте еще раз.", sending: "Отправка...", charCount: "символов" } },
        form: { sending: "Отправка...", success: "Мы получили ваш запрос. Спасибо.", error: "Произошла ошибка. Пожалуйста, попробуйте еще раз.", charCount: "символов" },
        footer: { description: "Расширьте свои возможности с инновационными AI приложениями и руководствами по строительству DIY домов на колёсах.", camping: { title: "Дом на колёсах", items: { videos: "Полный процесс DIY производства опубликован в видео", gallery: "Галерея завершенных примеров", guide: "Руководство по строительству DIY дома на колёсах", requirements: "Структурные требования дома на колёсах" } }, apps: "AI приложения", contact: "Контакты", info: { title: "Контактная информация", email: "Адрес электронной почты", youtube: "YouTube", tiktok: "TikTok", twitter: "X (ранее Twitter)" } }
    },
    
    // Chinese (Simplified) translations
    zh: {
        // Meta tags
        meta: {
            title: "TrailFusion AI - 房车DIY与AI应用开发",
            description: "从房车DIY制作到AI应用开发。将HiAce改装为自制房车的实用指南和创新AI应用介绍。"
        },
        
        // Navigation
        nav: {
            home: "首页",
            apps: "AI应用",
            camping: "房车制作",
            contact: "联系我们"
        },
        
        // Hero section
        hero: {
            line1: "自由旅行与",
            line2: "创新AI体验",
            line3: "的创造",
            description: "通过房车DIY和AI应用实现您的梦想",
            campingButton: "房车制作",
            appsButton: "AI应用",
            scrollText: "查看详情"
        },
        
        // Services section
        services: {
            title: "TrailFusion AI 服务",
            subtitle: "结合DIY专业知识与尖端AI技术，支持用户实现梦想",
            camping: {
                title: "HiAce DIY房车指南",
                description: "我们提供全面的指南，让初学者也能制作专业级房车。特别专注于HiAce改装，并提供包括8号牌照注册在内的全面支持。",
                features: {
                    f1: "详细制作手册",
                    f2: "所需材料和工具清单",
                    f3: "结构要求检查清单",
                    f4: "完成案例展示"
                },
                cta: "查看房车指南"
            },
            ai: {
                title: "创新AI应用",
                description: "我们开发并提供6个AI驱动的应用，以丰富日常生活和学习。运用最新AI技术，让用户的生活更便捷、更丰富。",
                features: {
                    f1: "GP Teachers: AI英语学习应用",
                    f2: "VoiceLink AI翻译: 实时翻译应用",
                    f3: "Word Blaster: 英语单词学习游戏",
                    f4: "其他包括辩论练习和工作计时器等"
                },
                cta: "查看应用"
            }
        },
        
        // About section
        about: {
            title: "代表介绍",
            motto: "人生，随时都能改变",
            subtitle: "要做的事现在就做，而不是等到老年",
            story: {
                p1: "我热爱日本。风情街景，疗愈身心的温泉，各地美食，还有温暖的人们。有朝一日，走遍这个心爱的日本是我的梦想。",
                p2: "然而，现实是忙碌的上班族。连续假期只是梦想，我告诉自己'日本一周游是退休后的乐趣'，几乎要放弃了。",
                p3: "然后，改变我人生180度的转折点到来了。那就是感染了新冠病毒。",
                p4: "独自一人，在房间里隔离了一周。面对死亡恐惧的情况下，我脑海中只有一个想法：'想做的事，现在不做就会后悔。人生太短暂了。'",
                p5: "能够走出房间的那一天，我的第二人生开始了。仅仅隔离的一周内，我就完成了制作自己房车所需的全部计划。",
                p6: "选择基础车型，设计四人家庭睡眠的滑动床，安排工作桌，确定符合法规的8号牌照注册要求，构建副电池系统，甚至内装用木材的采购。脑海中的设计图已经完全完成。",
                p7: "立即以170万日元购买了2008年HiAce Super Long（与救护车同尺寸！）。接下来两个月，平日下班后、周末、远程工作休息时间等，所有可用时间都献给了DIY。",
                p8: "然后迎来了目标的暑假。在与家人的首次车中泊旅行前，世界上独一无二的原创房车终于完成了。",
                p9: "那次旅行令人难以置信地感动。路过客满的酒店，在青森睡魔祭和长冈花火大会的前线体验狂热。在夏日北海道，不用担心飞涨的住宿费，享受了10天大自然，被孩子们满面笑容包围。",
                p10: "通过这次经历，我确信了：'一辆车竟能如此丰富人生。'"
            },
            additionalStory: {
                p11: "这次自由旅行给了我下一个、也是终生的梦想。那就是将这辆房车作为'环游世界的开发基地'，不受地点束缚，与世界各地的文化和人们交流，创造新价值。",
                p12: "具体来说，就是利用AI技术开发创新应用，通过Apple平台传递给全世界用户。将旅行中获得的灵感实时注入开发，让人们的生活更便利、更丰富。我找到了这种贡献的形式。",
                p13: "实现这个宏大愿景的旗帜，就是'Trail❌fusion❌AI'。",
                p14: "Trail（道路）：用房车环游世界的物理'道路'，以及通向无人走过的生活方式的人生'道路'。",
                p15: "Fusion（融合）：旅行这种真实体验与AI应用开发这种数字创造的'融合'。以及与世界各地用户想法的'融合'。",
                p16: "AI：开拓我们未来、创造无限可能的事业核心。",
                p17: "作为上班族几乎放弃的梦想。但这唯一的人生，不想再有遗憾。环游世界，创造价值，尽情享受相遇。总有一天，迎来人生终点时，想发自内心地笑着说'这是最棒的人生'。'TrailfusionAI'就是那个誓言的名字。",
                finalCta: "现在，您不也和我们一起，踏上这场名为无悔人生的冒险吗？"
            },
            callToAction: "现在，您不也和我们一起，踏上这场名为无悔人生的冒险吗？",
            followMe: "关注我",
            interests: "爱好与兴趣",
            tags: ["剑道", "房车", "DIY制作", "日本一周", "美食", "温泉", "应用开发", "马拉松", "登山"]
        },
        
        // Mobile navigation
        mobileNav: {
            home: "首页",
            camping: "房车制作",
            apps: "AI应用",
            contact: "联系我们"
        },
        
        // Contact section
        contact: {
            title: "联系我们",
            subtitle: "如有任何问题或咨询，请随时联系我们。",
            form: {
                name: "姓名",
                namePlaceholder: "张三",
                email: "电子邮件地址",
                emailPlaceholder: "example@mail.com",
                subject: "主题",
                subjectPlaceholder: "咨询内容主题",
                message: "消息",
                messagePlaceholder: "请填写您的问题或咨询内容",
                charCount: "字符",
                submit: "发送"
            },
            info: {
                title: "联系信息",
                email: "电子邮件地址",
                youtube: "TrailFusion AI 频道"
            },
            messages: {
                success: "我们已收到您的咨询。谢谢。",
                error: "发生错误。请重试。",
                sending: "发送中...",
                charCount: "字符"
            }
        },
        
        // Form messages and labels
        form: {
            sending: "发送中...",
            success: "我们已收到您的咨询。谢谢。",
            error: "发生错误。请重试。",
            charCount: "字符"
        },
        
        // Footer
        footer: {
            description: "通过创新AI应用和DIY房车制作指南拓展您的可能性。",
            camping: {
                title: "房车",
                items: {
                    videos: "DIY制作全过程视频公开",
                    gallery: "完成案例展示",
                    guide: "DIY房车制作指南",
                    requirements: "房车结构要求"
                }
            },
            apps: "AI应用",
            contact: "联系我们",
            info: {
                title: "联系信息",
                email: "电子邮件地址",
                youtube: "YouTube",
                tiktok: "TikTok",
                twitter: "X（原Twitter）"
            }
        }
    },
    
    // Korean translations
    ko: {
        // Meta tags
        meta: {
            title: "TrailFusion AI - 캠핑카 DIY 및 AI 애플리케이션 개발",
            description: "캠핑카 DIY 제작부터 AI 애플리케이션 개발까지. 하이에이스를 자작 캠핑카로 개조하는 실용 가이드와 혁신적인 AI 앱 소개."
        },
        
        // Navigation
        nav: {
            home: "홈",
            apps: "AI 앱",
            camping: "캠핑카 제작",
            contact: "문의하기"
        },
        
        // Hero section
        hero: {
            line1: "자유로운 여행과",
            line2: "혁신적인 AI 경험",
            line3: "을 창조하다",
            description: "캠핑카 DIY와 AI 애플리케이션으로 당신의 꿈을 현실로",
            campingButton: "캠핑카 제작",
            appsButton: "AI 앱",
            scrollText: "자세히 보기"
        },
        
        // Services section
        services: {
            title: "TrailFusion AI 서비스",
            subtitle: "DIY 전문 지식과 최첨단 AI 기술을 결합하여 사용자의 꿈 실현을 지원합니다",
            camping: {
                title: "하이에이스 DIY 캠핑 가이드",
                description: "초보자도 프로 수준의 캠핑카를 제작할 수 있도록 포괄적인 가이드를 제공합니다. 특히 하이에이스 개조에 중점을 두고 8번 번호판 등록까지 완벽하게 지원합니다.",
                features: {
                    f1: "상세한 제작 매뉴얼",
                    f2: "필요한 재료 및 도구 목록",
                    f3: "구조 요구사항 체크리스트",
                    f4: "완성 사례 갤러리"
                },
                cta: "캠핑 가이드 보기"
            },
            ai: {
                title: "혁신적인 AI 애플리케이션",
                description: "일상생활과 학습을 풍요롭게 하기 위한 AI 기반 6개 애플리케이션을 개발하고 제공합니다. 최신 AI 기술을 활용하여 사용자의 삶을 더욱 편리하고 풍요롭게 만듭니다.",
                features: {
                    f1: "GP Teachers: AI 영어 학습 앱",
                    f2: "VoiceLink AI 통역: 실시간 번역 앱",
                    f3: "Word Blaster: 영어 단어 학습 게임",
                    f4: "기타 토론 연습 및 작업 타이머 등"
                },
                cta: "앱 보기"
            }
        },
        
        // About section
        about: {
            title: "대표 소개",
            motto: "인생은 언제든지 바꿀 수 있다",
            subtitle: "하고 싶은 일은 노년이 아닌 지금 하겠다는 결심",
            story: {
                p1: "일본을 사랑했습니다. 정취 있는 거리 풍경, 마음과 몸을 치유하는 온천, 지역마다 맛볼 수 있는 미식, 그리고 따뜻한 사람들. 언젠가 이 사랑하는 일본을 구석구석 달려보는 것이 제 꿈이었습니다.",
                p2: "하지만 현실은 바쁜 직장인. 긴 휴가는 꿈도 꿀 수 없어 '일본 일주는 노후의 즐거움'이라고 스스로에게 말하며 포기하려 했습니다.",
                p3: "그런 제 인생이 180도 바뀌는 전환점이 찾아왔습니다. 신종 코로나바이러스 감염이었습니다.",
                p4: "혼자 방에 격리된 1주일. 죽음의 공포와 마주한 상황에서 제 머릿속을 스쳐간 것은 단 하나의 생각이었습니다. '하고 싶다고 생각한 것은 지금 당장 하지 않으면 후회한다. 인생은 너무나 짧다.'",
                p5: "방에서 나올 수 있게 된 그날, 제 두 번째 인생이 시작되었습니다. 격리된 단 1주일 동안 저는 캠핑카를 자작하기 위한 모든 계획을 세워 끝냈던 것입니다.",
                p6: "베이스 차량 선정, 4인 가족이 잘 수 있는 슬라이드 베드 설계, 일도 할 수 있는 책상 배치, 법규를 클리어하기 위한 8번 번호판 등록 요건 파악, 서브 배터리 시스템 구축, 그리고 내장에 사용할 목재 조달처까지. 머릿속 설계도는 완전히 완성되어 있었습니다.",
                p7: "즉시 2008년식 하이에이스 슈퍼 롱(구급차와 같은 크기!)을 170만 엔에 즉석 구매. 그로부터 2개월간 평일 퇴근 후, 주말, 재택근무 휴식 시간 등 사용할 수 있는 시간을 모두 DIY에 바쳤습니다.",
                p8: "그리고 목표로 했던 여름 휴가. 가족과의 첫 차박 여행을 앞두고 세상에 단 한 대뿐인 오리지널 캠핑카가 마침내 완성되었습니다.",
                p9: "그 여행은 상상을 초월할 정도로 감동적이었습니다. 만실인 호텔을 옆으로 하며 아오모리 네부타 축제와 나가오카 불꽃축제의 열광을 최전선에서 체감하기. 여름 홋카이도에서는 치솟는 숙박비를 걱정할 필요 없이 10일간 대자연을 만끽하며 아이들의 환한 웃음에 둘러싸이기.",
                p10: "이 경험을 통해 저는 확신했습니다. '한 대의 차가 이토록 인생을 풍요롭게 해줄 수 있구나'라고."
            },
            additionalStory: {
                p11: "그리고 이 자유로운 여행은 제게 다음, 그리고 평생을 걸 꿈을 주었습니다. 이 캠핑카를 '세계를 여행하는 개발 거점'으로 삼아 장소에 얽매이지 않고 세계 각지의 문화와 사람들과 교류하며 새로운 가치를 창조하는 것입니다.",
                p12: "구체적으로는 AI 기술을 활용한 혁신적인 애플리케이션을 개발하여 Apple 플랫폼을 통해 전 세계 사용자에게 전달하는 것. 여행지에서 얻은 영감을 실시간으로 개발에 쏟아 넣어 사람들의 삶을 더욱 편리하고 풍요롭게 만드는. 그런 기여의 형태를 찾았습니다.",
                p13: "이 장대한 비전을 실현하기 위한 깃발, 그것이 'Trail❌fusion❌AI'입니다.",
                p14: "Trail(길): 캠핑카로 세계를 여행하는 물리적인 '길'이며, 누구도 걸어본 적 없는 삶의 방식으로 이어지는 인생의 '길'.",
                p15: "Fusion(융합): 여행이라는 리얼한 체험과 AI 앱 개발이라는 디지털한 창조. 그리고 전 세계 사용자와 우리의 마음을 '융합'시키는 것.",
                p16: "AI: 우리의 미래를 개척하고 무한한 가능성을 창조하는 사업의 핵심.",
                p17: "직장인으로서 포기하려 했던 꿈. 하지만 단 한 번뿐인 인생, 더 이상 후회하고 싶지 않습니다. 전 세계를 여행하고 가치를 창조하며 만남을 마음껏 즐기고. 그리고 언젠가 인생의 마지막을 맞을 때 '최고의 인생이었다'고 진심으로 웃고 싶습니다. 'TrailfusionAI'는 그 맹세의 이름입니다.",
                finalCta: "자, 당신도 우리와 함께 후회 없는 인생이라는 이름의 모험을 떠나지 않겠습니까?"
            },
            callToAction: "자, 당신도 우리와 함께 후회 없는 인생이라는 이름의 모험을 떠나지 않겠습니까?",
            followMe: "팔로우하기",
            interests: "취미 및 관심사",
            tags: ["검도", "캠핑카", "DIY 제작", "일본 일주", "미식", "온천", "앱 개발", "마라톤", "등산"]
        },
        
        // Mobile navigation
        mobileNav: {
            home: "홈",
            camping: "캠핑카 제작",
            apps: "AI 앱",
            contact: "문의하기"
        },
        
        // Contact section
        contact: {
            title: "문의하기",
            subtitle: "질문이나 상담이 있으시면 언제든지 문의해 주세요.",
            form: {
                name: "이름",
                namePlaceholder: "홍길동",
                email: "이메일 주소",
                emailPlaceholder: "example@mail.com",
                subject: "제목",
                subjectPlaceholder: "문의 내용 제목",
                message: "메시지",
                messagePlaceholder: "질문이나 상담 내용을 입력해 주세요",
                charCount: "자",
                submit: "보내기"
            },
            info: {
                title: "연락처 정보",
                email: "이메일 주소",
                youtube: "TrailFusion AI 채널"
            },
            messages: {
                success: "문의를 접수했습니다. 감사합니다.",
                error: "오류가 발생했습니다. 다시 시도해 주세요.",
                sending: "전송 중...",
                charCount: "자"
            }
        },
        
        // Form messages and labels
        form: {
            sending: "전송 중...",
            success: "문의를 접수했습니다. 감사합니다.",
            error: "오류가 발생했습니다. 다시 시도해 주세요.",
            charCount: "자"
        },
        
        // Footer
        footer: {
            description: "혁신적인 AI 애플리케이션과 DIY 캠핑카 제작 가이드로 가능성을 넓혀보세요.",
            camping: {
                title: "캠핑카",
                items: {
                    videos: "DIY 제작 전 과정을 동영상으로 공개",
                    gallery: "완성 사례 갤러리",
                    guide: "DIY 캠핑카 제작 가이드",
                    requirements: "캠핑카 구조 요구사항"
                }
            },
            apps: "AI 앱",
            contact: "문의하기",
            info: {
                title: "연락처 정보",
                email: "이메일 주소",
                youtube: "YouTube",
                tiktok: "TikTok",
                twitter: "X (구 Twitter)"
            }
        }
    },
    
    // French translations
    fr: {
        // Meta tags
        meta: {
            title: "TrailFusion AI - DIY Camping-car et Développement d'Applications IA",
            description: "De la création DIY de camping-car au développement d'applications IA. Guide pratique pour convertir un HiAce en camping-car fait maison et présentation d'applications IA innovantes."
        },
        
        // Navigation
        nav: {
            home: "Accueil",
            apps: "Applications IA",
            camping: "Fabrication de Camping-car",
            contact: "Contact"
        },
        
        // Hero section
        hero: {
            line1: "Voyage libre et",
            line2: "Expérience IA innovante",
            line3: "Création",
            description: "Réalisez vos rêves avec le DIY camping-car et les applications IA",
            campingButton: "Fabrication de Camping-car",
            appsButton: "Applications IA",
            scrollText: "Voir les détails"
        },
        
        // Services section
        services: {
            title: "Services TrailFusion AI",
            subtitle: "Combinant l'expertise DIY avec la technologie IA de pointe pour soutenir la réalisation des rêves des utilisateurs",
            camping: {
                title: "Guide DIY Camping HiAce",
                description: "Nous fournissons des guides complets pour que même les débutants puissent construire des camping-cars de niveau professionnel. Nous nous concentrons particulièrement sur les modifications HiAce et fournissons un support complet jusqu'à l'enregistrement du numéro 8.",
                features: {
                    f1: "Manuel de construction détaillé",
                    f2: "Liste des matériaux et outils nécessaires",
                    f3: "Liste de vérification des exigences structurelles",
                    f4: "Galerie d'exemples terminés"
                },
                cta: "Voir le guide camping"
            },
            ai: {
                title: "Applications IA innovantes",
                description: "Nous développons et fournissons 6 applications basées sur l'IA pour enrichir la vie quotidienne et l'apprentissage. En utilisant la dernière technologie IA, nous rendons la vie des utilisateurs plus pratique et plus riche.",
                features: {
                    f1: "GP Teachers: Application d'apprentissage de l'anglais IA",
                    f2: "VoiceLink AI Interprète: Application de traduction en temps réel",
                    f3: "Word Blaster: Jeu d'apprentissage de vocabulaire anglais",
                    f4: "Autres incluent la pratique de débat et les minuteurs de travail"
                },
                cta: "Voir les applications"
            }
        },
        
        // About section
        about: {
            title: "Présentation du représentant",
            motto: "La vie peut toujours être changée",
            subtitle: "La détermination de faire ce que vous voulez maintenant, pas dans la vieillesse",
            story: {
                p1: "J'aimais le Japon. Les paysages urbains atmosphériques, les sources chaudes qui guérissent l'esprit et le corps, la cuisine locale, et les gens chaleureux. Un jour, voyager à travers ce Japon bien-aimé était mon rêve.",
                p2: "Cependant, la réalité était celle d'un salaryman occupé. Les vacances prolongées étaient un rêve impossible, et je me disais que 'faire le tour du Japon serait un plaisir pour la retraite' et j'abandonnais presque.",
                p3: "Puis vint un tournant qui changerait ma vie à 180 degrés. C'était mon infection par le nouveau coronavirus.",
                p4: "Seul, isolé dans une chambre pendant une semaine. Dans une situation face à la peur de la mort, une seule pensée traversa mon esprit : 'Si je veux faire quelque chose, je dois le faire maintenant ou je le regretterai. La vie est bien trop courte.'",
                p5: "Le jour où je pus sortir de ma chambre, ma seconde vie commença. En seulement une semaine d'isolement, j'avais fini de planifier tout ce qui était nécessaire pour construire mon propre camping-car.",
                p6: "Sélection du véhicule de base, conception d'un lit coulissant pour une famille de quatre personnes, arrangement d'un bureau pour le travail, identification des exigences d'enregistrement numéro 8 pour respecter les réglementations, construction d'un système de batterie auxiliaire, et même l'approvisionnement en bois pour l'intérieur. Le plan dans ma tête était complètement terminé.",
                p7: "J'ai immédiatement acheté un HiAce Super Long 2008 (même taille qu'une ambulance !) pour 1,7 million de yens. À partir de là, pendant deux mois, j'ai consacré tout le temps disponible au DIY - après le travail en semaine, les week-ends, et les pauses pendant le télétravail.",
                p8: "Et puis vinrent les vacances d'été que je visais. Avant le premier voyage de camping-car avec ma famille, le camping-car original unique au monde fut enfin terminé.",
                p9: "Ce voyage fut incroyablement émouvant. Passant devant des hôtels complets, vivant l'excitation du Festival Nebuta d'Aomori et du Festival de feux d'artifice de Nagaoka en première ligne. Dans l'Hokkaido estival, nous avons profité de la nature pendant 10 jours sans nous soucier des coûts d'hébergement qui montent en flèche, entourés des sourires radieux de nos enfants.",
                p10: "À travers cette expérience, j'ai été convaincu : 'Un véhicule peut-il enrichir la vie à ce point ?'"
            },
            additionalStory: {
                p11: "Et ce voyage libre m'a donné le rêve suivant, et celui pour lequel je mise ma vie. C'est faire de ce camping-car une 'base de développement pour voyager dans le monde', créant de nouvelles valeurs tout en interagissant avec les cultures et les gens du monde entier, sans être lié par l'emplacement.",
                p12: "Spécifiquement, développer des applications innovantes utilisant la technologie IA et les livrer aux utilisateurs du monde entier via la plateforme Apple. Verser l'inspiration obtenue des voyages dans le développement en temps réel, rendant la vie des gens plus pratique et plus riche. J'ai trouvé cette forme de contribution.",
                p13: "Le drapeau pour réaliser cette vision grandiose, c'est 'Trail❌fusion❌AI'.",
                p14: "Trail (Route) : La 'route' physique de voyager dans le monde avec un camping-car, et la 'route' de vie menant à un mode de vie que personne n'a jamais parcouru.",
                p15: "Fusion : La 'fusion' de l'expérience réelle qu'est le voyage avec la création numérique qu'est le développement d'applications IA. Et 'fusionner' les pensées des utilisateurs du monde entier avec les nôtres.",
                p16: "AI : Le cœur de notre entreprise qui ouvre notre avenir et crée des possibilités infinies.",
                p17: "Rêves que j'avais presque abandonnés en tant que salaryman. Mais dans cette vie unique, je ne veux plus avoir de regrets. Voyager dans le monde, créer de la valeur, et profiter pleinement des rencontres. Et un jour, quand je ferai face à la fin de ma vie, je veux sourire du fond du cœur en disant 'C'était la meilleure vie'. 'TrailfusionAI' est le nom de ce serment.",
                finalCta: "Maintenant, ne vous joindrez-vous pas à nous dans une aventure appelée une vie sans regrets ?"
            },
            callToAction: "Maintenant, ne vous joindrez-vous pas à nous dans une aventure appelée une vie sans regrets ?",
            followMe: "Suivez-moi",
            interests: "Loisirs et Intérêts",
            tags: ["Kendo", "Camping-car", "Fabrication DIY", "Tour du Japon", "Gastronomie", "Sources Chaudes", "Développement d'Applications", "Marathon", "Escalade"]
        },
        
        // Mobile navigation
        mobileNav: {
            home: "Accueil",
            camping: "Fabrication de Camping-car",
            apps: "Applications IA",
            contact: "Contact"
        },
        
        // Contact section
        contact: {
            title: "Contact",
            subtitle: "Si vous avez des questions ou des consultations, n'hésitez pas à nous contacter.",
            form: {
                name: "Nom",
                namePlaceholder: "Jean Dupont",
                email: "Adresse e-mail",
                emailPlaceholder: "example@mail.com",
                subject: "Sujet",
                subjectPlaceholder: "Sujet de votre demande",
                message: "Message",
                messagePlaceholder: "Veuillez écrire vos questions ou le contenu de votre consultation",
                charCount: "caractères",
                submit: "Envoyer"
            },
            info: {
                title: "Informations de contact",
                email: "Adresse e-mail",
                youtube: "Chaîne TrailFusion AI"
            },
            messages: {
                success: "Nous avons reçu votre demande. Merci.",
                error: "Une erreur s'est produite. Veuillez réessayer.",
                sending: "Envoi en cours...",
                charCount: "caractères"
            }
        },
        
        // Form messages and labels
        form: {
            sending: "Envoi en cours...",
            success: "Nous avons reçu votre demande. Merci.",
            error: "Une erreur s'est produite. Veuillez réessayer.",
            charCount: "caractères"
        },
        
        // Footer
        footer: {
            description: "Élargissez vos possibilités avec des applications IA innovantes et des guides de construction de camping-car DIY.",
            camping: {
                title: "Camping-car",
                items: {
                    videos: "Processus complet de fabrication DIY publié en vidéos",
                    gallery: "Galerie d'exemples terminés",
                    guide: "Guide de construction de camping-car DIY",
                    requirements: "Exigences structurelles de camping-car"
                }
            },
            apps: "Applications IA",
            contact: "Contact",
            info: {
                title: "Informations de contact",
                email: "Adresse e-mail",
                youtube: "YouTube",
                tiktok: "TikTok",
                twitter: "X (anciennement Twitter)"
            }
        }
    },
    
    // Portuguese translations
    pt: {
        meta: { title: "TrailFusion AI - DIY de Motorhome e Desenvolvimento de Aplicações IA", description: "Da criação DIY de motorhome ao desenvolvimento de aplicações IA. Guia prático para converter um HiAce em motorhome caseiro e apresentação de aplicações IA inovadoras." },
        nav: { home: "Início", apps: "Apps IA", camping: "Fabricação de Motorhome", contact: "Contato" },
        hero: { line1: "Viagem livre e", line2: "Experiência IA inovadora", line3: "Criação", description: "Realize seus sonhos com DIY de motorhome e aplicações IA", campingButton: "Fabricação de Motorhome", appsButton: "Apps IA", scrollText: "Ver detalhes" },
        services: { title: "Serviços TrailFusion AI", subtitle: "Combinando expertise DIY com tecnologia IA de ponta para apoiar a realização dos sonhos dos usuários", camping: { title: "Guia DIY Camping HiAce", description: "Fornecemos guias abrangentes para que até iniciantes possam construir motorhomes de nível profissional. Focamos particularmente em modificações HiAce e fornecemos suporte completo até o registro de número 8.", features: { f1: "Manual de construção detalhado", f2: "Lista de materiais e ferramentas necessárias", f3: "Lista de verificação de requisitos estruturais", f4: "Galeria de exemplos concluídos" }, cta: "Ver guia de camping" }, ai: { title: "Aplicações IA inovadoras", description: "Desenvolvemos e fornecemos 6 aplicações baseadas em IA para enriquecer a vida cotidiana e o aprendizado. Usando a mais recente tecnologia IA, tornamos a vida dos usuários mais conveniente e rica.", features: { f1: "GP Teachers: App de aprendizado de inglês IA", f2: "VoiceLink AI Intérprete: App de tradução em tempo real", f3: "Word Blaster: Jogo de aprendizado de vocabulário inglês", f4: "Outros incluem prática de debate e cronômetros de trabalho" }, cta: "Ver aplicações" } },
        about: { title: "Apresentação do representante", motto: "A vida sempre pode ser mudada", subtitle: "A determinação de fazer o que você quer agora, não na velhice", story: { p1: "Eu amava o Japão. As paisagens urbanas atmosféricas, fontes termais que curam mente e corpo, culinária local e pessoas calorosas. Algum dia, viajar por este Japão amado era meu sonho.", p2: "No entanto, a realidade era de um assalariado ocupado. Férias prolongadas eram um sonho impossível, e eu me dizia que 'viajar pelo Japão seria um prazer para a aposentadoria' e quase desistia.", p3: "Então veio um ponto de virada que mudaria minha vida 180 graus. Foi minha infecção pelo novo coronavírus.", p4: "Sozinho, isolado em um quarto por uma semana. Em uma situação enfrentando o medo da morte, apenas um pensamento passou pela minha mente: 'Se eu quero fazer algo, devo fazer agora ou me arrependerei. A vida é muito curta.'", p5: "No dia em que pude sair do meu quarto, minha segunda vida começou. Em apenas uma semana de isolamento, eu havia terminado de planejar tudo o que era necessário para construir meu próprio motorhome.", p6: "Seleção do veículo base, projeto de cama deslizante para família de quatro pessoas, arranjo de mesa para trabalho, identificação de requisitos de registro número 8 para atender regulamentações, construção de sistema de bateria auxiliar, e até fornecimento de madeira para o interior. O projeto na minha cabeça estava completamente terminado.", p7: "Imediatamente comprei um HiAce Super Long 2008 (mesmo tamanho de uma ambulância!) por 1,7 milhão de ienes. A partir daí, por dois meses, dediquei todo o tempo disponível ao DIY - após o trabalho nos dias de semana, fins de semana e pausas durante o trabalho remoto.", p8: "E então chegaram as férias de verão que eu almejava. Antes da primeira viagem de acampamento com minha família, o motorhome original único no mundo finalmente foi concluído.", p9: "Essa viagem foi incrivelmente emocionante. Passando por hotéis lotados, vivenciando a emoção do Festival Nebuta de Aomori e do Festival de Fogos de Artifício de Nagaoka na linha de frente. No Hokkaido de verão, aproveitamos a natureza por 10 dias sem nos preocupar com os custos de acomodação em alta, cercados pelos sorrisos radiantes de nossos filhos.", p10: "Através dessa experiência, fiquei convencido: 'Um veículo pode enriquecer a vida a este ponto?'" }, additionalStory: { p11: "E essa viagem livre me deu o próximo sonho, e aquele no qual apostaria minha vida. É fazer deste motorhome uma 'base de desenvolvimento para viajar pelo mundo', criando novos valores enquanto interage com culturas e pessoas ao redor do mundo, sem ser limitado pela localização.", p12: "Especificamente, desenvolver aplicações inovadoras usando tecnologia IA e entregá-las aos usuários em todo o mundo via plataforma Apple. Derramando inspiração obtida de viagens no desenvolvimento em tempo real, tornando a vida das pessoas mais conveniente e rica. Encontrei essa forma de contribuição.", p13: "A bandeira para realizar essa visão grandiosa é 'Trail❌fusion❌AI'.", p14: "Trail (Trilha): A 'trilha' física de viajar pelo mundo com um motorhome, e a 'trilha' de vida levando a um modo de vida que ninguém jamais percorreu.", p15: "Fusion (Fusão): A 'fusão' da experiência real que é a viagem com a criação digital que é o desenvolvimento de aplicações IA. E 'fundir' os pensamentos dos usuários ao redor do mundo com os nossos.", p16: "AI: O núcleo do nosso negócio que abre nosso futuro e cria possibilidades infinitas.", p17: "Sonhos que quase abandonei como assalariado. Mas nesta vida única, não quero mais ter arrependimentos. Viajar pelo mundo, criar valor e desfrutar plenamente dos encontros. E algum dia, quando enfrentar o fim da minha vida, quero sorrir do fundo do coração dizendo 'Foi a melhor vida'. 'TrailfusionAI' é o nome desse juramento.", finalCta: "Agora, você não se juntará a nós em uma aventura chamada vida sem arrependimentos?" }, callToAction: "Agora, você não se juntará a nós em uma aventura chamada vida sem arrependimentos?", followMe: "Siga-me", interests: "Hobbies e Interesses", tags: ["Kendo", "Motorhome", "Fabricação DIY", "Tour pelo Japão", "Gastronomia", "Fontes Termais", "Desenvolvimento de Apps", "Maratona", "Montanhismo"] },
        mobileNav: { home: "Início", camping: "Fabricação de Motorhome", apps: "Apps IA", contact: "Contato" },
        contact: { title: "Contato", subtitle: "Se você tem alguma pergunta ou consulta, sinta-se à vontade para nos contatar.", form: { name: "Nome", namePlaceholder: "João Silva", email: "Endereço de e-mail", emailPlaceholder: "example@mail.com", subject: "Assunto", subjectPlaceholder: "Assunto da sua consulta", message: "Mensagem", messagePlaceholder: "Por favor, escreva suas perguntas ou conteúdo de consulta", charCount: "caracteres", submit: "Enviar" }, info: { title: "Informações de contato", email: "Endereço de e-mail", youtube: "Canal TrailFusion AI" }, messages: { success: "Recebemos sua consulta. Obrigado.", error: "Ocorreu um erro. Tente novamente.", sending: "Enviando...", charCount: "caracteres" } },
        form: { sending: "Enviando...", success: "Recebemos sua consulta. Obrigado.", error: "Ocorreu um erro. Tente novamente.", charCount: "caracteres" },
        footer: { description: "Expanda suas possibilidades com aplicações IA inovadoras e guias de construção de motorhome DIY.", camping: { title: "Motorhome", items: { videos: "Processo completo de fabricação DIY publicado em vídeos", gallery: "Galeria de exemplos concluídos", guide: "Guia de construção de motorhome DIY", requirements: "Requisitos estruturais de motorhome" } }, apps: "Apps IA", contact: "Contato", info: { title: "Informações de contato", email: "Endereço de e-mail", youtube: "YouTube", tiktok: "TikTok", twitter: "X (antigo Twitter)" } }
    },
    
    // Hindi translations  
    hi: {
        meta: { title: "TrailFusion AI - कैम्पर DIY और AI एप्लिकेशन डेवलपमेंट", description: "कैम्पर DIY निर्माण से AI एप्लिकेशन डेवलपमेंट तक। HiAce को घर में बने कैम्पर में बदलने की व्यावहारिक गाइड और नवाचार AI ऐप्स का परिचय।" },
        nav: { home: "होम", apps: "AI ऐप्स", camping: "कैम्पर निर्माण", contact: "संपर्क करें" },
        hero: { line1: "स्वतंत्र यात्रा और", line2: "नवाचार AI अनुभव", line3: "का निर्माण", description: "कैम्पर DIY और AI एप्लिकेशन के साथ अपने सपनों को साकार करें", campingButton: "कैम्पर निर्माण", appsButton: "AI ऐप्स", scrollText: "विवरण देखें" },
        services: { title: "TrailFusion AI सेवाएं", subtitle: "उपयोगकर्ताओं के सपनों की प्राप्ति का समर्थन करने के लिए DIY विशेषज्ञता को अत्याधुनिक AI तकनीक के साथ मिलाना", camping: { title: "HiAce DIY कैम्पिंग गाइड", description: "हम व्यापक गाइड प्रदान करते हैं ताकि शुरुआती भी पेशेवर स्तर के कैम्पर बना सकें। हम विशेष रूप से HiAce संशोधनों पर ध्यान देते हैं और 8-नंबर पंजीकरण तक पूर्ण सहायता प्रदान करते हैं।", features: { f1: "विस्तृत निर्माण मैनुअल", f2: "आवश्यक सामग्री और उपकरणों की सूची", f3: "संरचनात्मक आवश्यकताओं की जांच सूची", f4: "पूर्ण उदाहरण गैलरी" }, cta: "कैम्पिंग गाइड देखें" }, ai: { title: "नवाचार AI एप्लिकेशन", description: "हम दैनिक जीवन और सीखने को समृद्ध बनाने के लिए AI-संचालित 6 एप्लिकेशन विकसित और प्रदान करते हैं। नवीनतम AI तकनीक का उपयोग करके, हम उपयोगकर्ताओं के जीवन को अधिक सुविधाजनक और समृद्ध बनाते हैं।", features: { f1: "GP Teachers: AI अंग्रेजी सीखने का ऐप", f2: "VoiceLink AI दुभाषिया: रियल-टाइम अनुवाद ऐप", f3: "Word Blaster: अंग्रेजी शब्दावली सीखने का गेम", f4: "अन्य में बहस अभ्यास और कार्य टाइमर शामिल हैं" }, cta: "ऐप्स देखें" } },
        about: { title: "प्रतिनिधि परिचय", motto: "जीवन हमेशा बदला जा सकता है", subtitle: "जो करना चाहते हैं वह बुढ़ापे में नहीं बल्कि अभी करने का संकल्प", story: { p1: "मैं जापान से प्यार करता था। वायुमंडलीय शहरी दृश्य, मन और शरीर को ठीक करने वाले गर्म झरने, स्थानीय व्यंजन, और गर्मजोशी भरे लोग। कभी इस प्रिय जापान की यात्रा करना मेरा सपना था।", p2: "हालांकि, वास्तविकता एक व्यस्त वेतनभोगी की थी। विस्तृत छुट्टियां एक असंभव सपना थीं, और मैं खुद से कहता था कि 'जापान का दौरा सेवानिवृत्ति के लिए एक खुशी होगी' और लगभग हार मान रहा था।", p3: "फिर एक मोड़ आया जो मेरे जीवन को 180 डिग्री बदल देगा। यह नए कोरोनावायरस से मेरा संक्रमण था।", p4: "अकेले, एक कमरे में एक सप्ताह के लिए अलग। मृत्यु के डर से सामना करने की स्थिति में, केवल एक विचार मेरे मन में आया: 'अगर मैं कुछ करना चाहता हूं, तो मुझे इसे अभी करना चाहिए या मुझे पछतावा होगा। जीवन बहुत छोटा है।'", p5: "जिस दिन मैं अपने कमरे से निकल सका, मेरा दूसरा जीवन शुरू हुआ। केवल एक सप्ताह के अलगाव में, मैंने अपना खुद का कैम्पर बनाने के लिए आवश्यक सब कुछ योजना बना ली थी।", p6: "आधार वाहन का चयन, चार लोगों के परिवार के लिए स्लाइड बेड का डिजाइन, काम के लिए डेस्क की व्यवस्था, नियमों को पूरा करने के लिए 8-नंबर पंजीकरण आवश्यकताओं की पहचान, सब-बैटरी सिस्टम का निर्माण, और यहां तक कि इंटीरियर के लिए लकड़ी की आपूर्ति। मेरे सिर में ब्लूप्रिंट पूरी तरह से तैयार था।", p7: "मैंने तुरंत 2008 HiAce Super Long (एंबुलेंस के समान आकार!) को 1.7 मिलियन येन में खरीदा। वहां से, दो महीने तक, मैंने उपलब्ध सभी समय DIY को समर्पित किया - सप्ताह के दिनों में काम के बाद, सप्ताहांत, और रिमोट वर्क के दौरान ब्रेक।", p8: "और फिर गर्मी की छुट्टियां आईं जिसका मैं लक्ष्य बना रहा था। अपने परिवार के साथ पहली कार कैम्पिंग यात्रा से पहले, दुनिया में अनूठा मूल कैम्पर अंततः पूरा हो गया।", p9: "वह यात्रा अविश्वसनीय रूप से भावनात्मक थी। भरे हुए होटलों को पार करते हुए, अओमोरी नेबुता फेस्टिवल और नागाओका आतिशबाजी महोत्सव के उत्साह को अग्रिम पंक्ति में अनुभव करना। गर्मियों के होक्काइडो में, हमने बढ़ती आवास लागतों की चिंता किए बिना 10 दिनों तक प्रकृति का आनंद लिया, अपने बच्चों की चमकती मुस्कान से घिरे हुए।", p10: "इस अनुभव के माध्यम से, मैं आश्वस्त हो गया: 'क्या एक वाहन जीवन को इस हद तक समृद्ध बना सकता है?'" }, additionalStory: { p11: "और इस स्वतंत्र यात्रा ने मुझे अगला सपना दिया, और वह जिस पर मैं अपना जीवन दांव पर लगाऊंगा। यह इस कैम्पर को 'दुनिया की यात्रा के लिए विकास आधार' बनाना है, स्थान से बंधे बिना दुनिया भर की संस्कृतियों और लोगों के साथ बातचीत करते हुए नए मूल्य बनाना।", p12: "विशेष रूप से, AI तकनीक का उपयोग करके नवाचार एप्लिकेशन विकसित करना और Apple प्लेटफॉर्म के माध्यम से दुनिया भर के उपयोगकर्ताओं को वितरित करना। यात्राओं से प्राप्त प्रेरणा को वास्तविक समय में विकास में डालना, लोगों के जीवन को अधिक सुविधाजनक और समृद्ध बनाना। मैंने योगदान के इस रूप को पाया।", p13: "इस भव्य दृष्टि को साकार करने का झंडा 'Trail❌fusion❌AI' है।", p14: "Trail (पथ): कैम्पर के साथ दुनिया की यात्रा का भौतिक 'पथ', और जीवन का 'पथ' जो ऐसी जीवनशैली की ओर ले जाता है जिसे किसी ने कभी नहीं चला।", p15: "Fusion (संलयन): यात्रा के वास्तविक अनुभव का AI ऐप विकास के डिजिटल निर्माण के साथ 'संलयन'। और दुनिया भर के उपयोगकर्ताओं के विचारों को हमारे साथ 'मिलाना'।", p16: "AI: हमारे व्यवसाय का मूल जो हमारे भविष्य को खोलता है और अनंत संभावनाएं बनाता है।", p17: "सपने जिन्हें मैंने वेतनभोगी के रूप में लगभग छोड़ दिया था। लेकिन इस एकमात्र जीवन में, मैं और पछतावा नहीं चाहता। दुनिया की यात्रा करना, मूल्य बनाना, और मुलाकातों का पूरा आनंद लेना। और कभी जब मैं अपने जीवन के अंत का सामना करूंगा, मैं दिल से मुस्कराना चाहता हूं कहते हुए 'यह सबसे अच्छा जीवन था'। 'TrailfusionAI' उस शपथ का नाम है।", finalCta: "अब, क्या आप हमारे साथ बिना पछतावे के जीवन नामक एक साहसिक कार्य में शामिल नहीं होंगे?" }, callToAction: "अब, क्या आप हमारे साथ बिना पछतावे के जीवन नामक एक साहसिक कार्य में शामिल नहीं होंगे?", followMe: "मुझे फॉलो करें", interests: "शौक और रुचियां", tags: ["केंडो", "कैम्पर", "DIY निर्माण", "जापान दौरा", "भोजन", "गर्म झरने", "ऐप विकास", "मैराथन", "पर्वतारोहण"] },
        mobileNav: { home: "होम", camping: "कैम्पर निर्माण", apps: "AI ऐप्स", contact: "संपर्क करें" },
        contact: { title: "संपर्क करें", subtitle: "यदि आपके कोई प्रश्न या परामर्श हैं, तो कृपया बेझिझक हमसे संपर्क करें।", form: { name: "नाम", namePlaceholder: "राम शर्मा", email: "ईमेल पता", emailPlaceholder: "example@mail.com", subject: "विषय", subjectPlaceholder: "आपकी पूछताछ का विषय", message: "संदेश", messagePlaceholder: "कृपया अपने प्रश्न या परामर्श सामग्री लिखें", charCount: "अक्षर", submit: "भेजें" }, info: { title: "संपर्क जानकारी", email: "ईमेल पता", youtube: "TrailFusion AI चैनल" }, messages: { success: "हमें आपकी पूछताछ मिली है। धन्यवाद।", error: "एक त्रुटि हुई। कृपया पुनः प्रयास करें।", sending: "भेजा जा रहा है...", charCount: "अक्षर" } },
        form: { sending: "भेजा जा रहा है...", success: "हमें आपकी पूछताछ मिली है। धन्यवाद।", error: "एक त्रुटि हुई। कृपया पुनः प्रयास करें।", charCount: "अक्षर" },
        footer: { description: "नवाचार AI एप्लिकेशन और DIY कैम्पर निर्माण गाइड के साथ अपनी संभावनाओं का विस्तार करें।", camping: { title: "कैम्पर", items: { videos: "पूर्ण DIY निर्माण प्रक्रिया वीडियो में प्रकाशित", gallery: "पूर्ण उदाहरण गैलरी", guide: "DIY कैम्पर निर्माण गाइड", requirements: "कैम्पर संरचनात्मक आवश्यकताएं" } }, apps: "AI ऐप्स", contact: "संपर्क करें", info: { title: "संपर्क जानकारी", email: "ईमेल पता", youtube: "YouTube", tiktok: "TikTok", twitter: "X (पूर्व में Twitter)" } }
    },
    
    // German translations
    de: {
        meta: { title: "TrailFusion AI - Wohnmobil DIY & KI-Anwendungsentwicklung", description: "Von Wohnmobil-DIY-Erstellung bis zur KI-Anwendungsentwicklung. Praktischer Leitfaden zur Umwandlung eines HiAce in ein selbstgebautes Wohnmobil und Vorstellung innovativer KI-Apps." },
        nav: { home: "Startseite", apps: "KI-Apps", camping: "Wohnmobil-Herstellung", contact: "Kontakt" },
        hero: { line1: "Freies Reisen und", line2: "Innovative KI-Erfahrung", line3: "Erschaffung", description: "Verwirklichen Sie Ihre Träume mit Wohnmobil-DIY und KI-Anwendungen", campingButton: "Wohnmobil-Herstellung", appsButton: "KI-Apps", scrollText: "Details anzeigen" },
        services: { title: "TrailFusion AI Dienstleistungen", subtitle: "Kombination von DIY-Expertise mit modernster KI-Technologie zur Unterstützung der Traumverwirklichung der Nutzer", camping: { title: "HiAce DIY Camping Leitfaden", description: "Wir bieten umfassende Leitfäden, damit auch Anfänger professionelle Wohnmobile bauen können. Wir konzentrieren uns besonders auf HiAce-Modifikationen und bieten vollständige Unterstützung bis zur 8-Nummer-Registrierung.", features: { f1: "Detailliertes Konstruktionshandbuch", f2: "Liste der benötigten Materialien und Werkzeuge", f3: "Checkliste für strukturelle Anforderungen", f4: "Galerie abgeschlossener Beispiele" }, cta: "Camping-Leitfaden anzeigen" }, ai: { title: "Innovative KI-Anwendungen", description: "Wir entwickeln und bieten 6 KI-gestützte Anwendungen zur Bereicherung des täglichen Lebens und Lernens. Mit modernster KI-Technologie machen wir das Leben der Nutzer bequemer und reicher.", features: { f1: "GP Teachers: KI-Englisch-Lern-App", f2: "VoiceLink KI-Dolmetscher: Echtzeit-Übersetzungs-App", f3: "Word Blaster: Englisch-Vokabel-Lernspiel", f4: "Andere umfassen Debattierübung und Arbeits-Timer" }, cta: "Apps anzeigen" } },
        about: { title: "Vertreter-Vorstellung", motto: "Das Leben kann immer verändert werden", subtitle: "Die Entschlossenheit, das zu tun, was man will, jetzt und nicht im Alter", story: { p1: "Ich liebte Japan. Die atmosphärischen Stadtlandschaften, heiße Quellen, die Geist und Körper heilen, lokale Küche und herzliche Menschen. Eines Tages durch dieses geliebte Japan zu reisen war mein Traum.", p2: "Die Realität war jedoch die eines beschäftigten Angestellten. Längere Ferien waren ein unmöglicher Traum, und ich sagte mir, dass 'eine Japan-Rundreise ein Vergnügen für den Ruhestand wäre' und gab fast auf.", p3: "Dann kam ein Wendepunkt, der mein Leben um 180 Grad verändern würde. Es war meine Infektion mit dem neuen Coronavirus.", p4: "Allein, eine Woche lang in einem Zimmer isoliert. In einer Situation, die der Todesangst gegenüberstand, ging mir nur ein Gedanke durch den Kopf: 'Wenn ich etwas tun will, muss ich es jetzt tun oder ich werde es bereuen. Das Leben ist zu kurz.'", p5: "Am Tag, als ich mein Zimmer verlassen konnte, begann mein zweites Leben. In nur einer Woche Isolation hatte ich alles geplant, was nötig war, um mein eigenes Wohnmobil zu bauen.", p6: "Auswahl des Basisfahrzeugs, Entwurf eines Schiebebetts für eine vierköpfige Familie, Anordnung eines Schreibtischs für die Arbeit, Identifizierung der 8-Nummer-Registrierungsanforderungen zur Einhaltung der Vorschriften, Aufbau eines Hilfsbatteriesystems und sogar Beschaffung von Holz für das Interieur. Der Plan in meinem Kopf war vollständig fertig.", p7: "Ich kaufte sofort einen 2008 HiAce Super Long (gleiche Größe wie ein Krankenwagen!) für 1,7 Millionen Yen. Von da an widmete ich zwei Monate lang alle verfügbare Zeit dem DIY - nach der Arbeit an Wochentagen, Wochenenden und Pausen während der Heimarbeit.", p8: "Und dann kamen die Sommerferien, die ich angestrebt hatte. Vor der ersten Campingreise mit meiner Familie war das einzigartige Original-Wohnmobil der Welt endlich fertiggestellt.", p9: "Diese Reise war unglaublich bewegend. An ausgebuchten Hotels vorbei, die Aufregung des Aomori Nebuta Festivals und des Nagaoka Feuerwerk Festivals an vorderster Front erleben. Im sommerlichen Hokkaido genossen wir 10 Tage lang die Natur, ohne uns um steigende Unterkunftskosten zu sorgen, umgeben von den strahlenden Lächeln unserer Kinder.", p10: "Durch diese Erfahrung wurde ich überzeugt: 'Kann ein Fahrzeug das Leben so sehr bereichern?'" }, additionalStory: { p11: "Und diese freie Reise gab mir den nächsten Traum, und den, für den ich mein Leben einsetzen würde. Es ist, dieses Wohnmobil zu einer 'Entwicklungsbasis für Weltreisen' zu machen, neue Werte zu schaffen, während man mit Kulturen und Menschen auf der ganzen Welt interagiert, ohne an einen Ort gebunden zu sein.", p12: "Konkret innovative Anwendungen mit KI-Technologie zu entwickeln und sie über die Apple-Plattform an Nutzer weltweit zu liefern. Inspiration aus Reisen in Echtzeit in die Entwicklung zu gießen, das Leben der Menschen bequemer und reicher zu machen. Ich fand diese Form des Beitrags.", p13: "Die Flagge zur Verwirklichung dieser großartigen Vision ist 'Trail❌fusion❌AI'.", p14: "Trail (Pfad): Der physische 'Pfad' des Weltreisens mit einem Wohnmobil und der Lebens-'Pfad', der zu einer Lebensweise führt, die niemand je gegangen ist.", p15: "Fusion: Die 'Fusion' der realen Erfahrung des Reisens mit der digitalen Schöpfung der KI-App-Entwicklung. Und die Gedanken der Nutzer weltweit mit unseren zu 'verschmelzen'.", p16: "AI: Der Kern unseres Geschäfts, der unsere Zukunft öffnet und unendliche Möglichkeiten schafft.", p17: "Träume, die ich als Angestellter fast aufgegeben hatte. Aber in diesem einen Leben will ich keine Reue mehr haben. Die Welt bereisen, Werte schaffen und Begegnungen voll auskosten. Und eines Tages, wenn ich dem Ende meines Lebens gegenüberstehe, möchte ich von Herzen lächeln und sagen 'Es war das beste Leben'. 'TrailfusionAI' ist der Name dieses Schwurs.", finalCta: "Werden Sie sich uns nun nicht auf einem Abenteuer namens Leben ohne Reue anschließen?" }, callToAction: "Werden Sie sich uns nun nicht auf einem Abenteuer namens Leben ohne Reue anschließen?", followMe: "Folgen Sie mir", interests: "Hobbys & Interessen", tags: ["Kendo", "Wohnmobil", "DIY-Herstellung", "Japan-Tour", "Gourmet", "Heiße Quellen", "App-Entwicklung", "Marathon", "Bergsteigen"] },
        mobileNav: { home: "Startseite", camping: "Wohnmobil-Herstellung", apps: "KI-Apps", contact: "Kontakt" },
        contact: { title: "Kontakt", subtitle: "Wenn Sie Fragen oder Beratung haben, zögern Sie nicht, uns zu kontaktieren.", form: { name: "Name", namePlaceholder: "Max Mustermann", email: "E-Mail-Adresse", emailPlaceholder: "example@mail.com", subject: "Betreff", subjectPlaceholder: "Betreff Ihrer Anfrage", message: "Nachricht", messagePlaceholder: "Bitte schreiben Sie Ihre Fragen oder Beratungsinhalte", charCount: "Zeichen", submit: "Senden" }, info: { title: "Kontaktinformationen", email: "E-Mail-Adresse", youtube: "TrailFusion AI Kanal" }, messages: { success: "Wir haben Ihre Anfrage erhalten. Vielen Dank.", error: "Ein Fehler ist aufgetreten. Bitte versuchen Sie es erneut.", sending: "Wird gesendet...", charCount: "Zeichen" } },
        form: { sending: "Wird gesendet...", success: "Wir haben Ihre Anfrage erhalten. Vielen Dank.", error: "Ein Fehler ist aufgetreten. Bitte versuchen Sie es erneut.", charCount: "Zeichen" },
        footer: { description: "Erweitern Sie Ihre Möglichkeiten mit innovativen KI-Anwendungen und DIY-Wohnmobil-Bauleitfäden.", camping: { title: "Wohnmobil", items: { videos: "Vollständiger DIY-Herstellungsprozess in Videos veröffentlicht", gallery: "Galerie abgeschlossener Beispiele", guide: "DIY-Wohnmobil-Bauleitfaden", requirements: "Wohnmobil-Strukturanforderungen" } }, apps: "KI-Apps", contact: "Kontakt", info: { title: "Kontaktinformationen", email: "E-Mail-Adresse", youtube: "YouTube", tiktok: "TikTok", twitter: "X (ehemals Twitter)" } }
    },
    
    // Spanish translations
    es: {
        meta: { title: "TrailFusion AI - DIY de Autocaravana y Desarrollo de Aplicaciones IA", description: "Desde la creación DIY de autocaravanas hasta el desarrollo de aplicaciones IA. Guía práctica para convertir un HiAce en autocaravana casera y presentación de aplicaciones IA innovadoras." },
        nav: { home: "Inicio", apps: "Apps IA", camping: "Fabricación de Autocaravana", contact: "Contacto" },
        hero: { line1: "Viaje libre y", line2: "Experiencia IA innovadora", line3: "Creación", description: "Haz realidad tus sueños con DIY de autocaravana y aplicaciones IA", campingButton: "Fabricación de Autocaravana", appsButton: "Apps IA", scrollText: "Ver detalles" },
        services: { title: "Servicios TrailFusion AI", subtitle: "Combinando experiencia DIY con tecnología IA de vanguardia para apoyar la realización de los sueños de los usuarios", camping: { title: "Guía DIY Camping HiAce", description: "Proporcionamos guías completas para que incluso los principiantes puedan construir autocaravanas de nivel profesional. Nos enfocamos particularmente en modificaciones HiAce y proporcionamos soporte completo hasta el registro de número 8.", features: { f1: "Manual de construcción detallado", f2: "Lista de materiales y herramientas necesarias", f3: "Lista de verificación de requisitos estructurales", f4: "Galería de ejemplos completados" }, cta: "Ver guía de camping" }, ai: { title: "Aplicaciones IA innovadoras", description: "Desarrollamos y proporcionamos 6 aplicaciones impulsadas por IA para enriquecer la vida diaria y el aprendizaje. Usando la última tecnología IA, hacemos la vida de los usuarios más conveniente y rica.", features: { f1: "GP Teachers: App de aprendizaje de inglés IA", f2: "VoiceLink AI Intérprete: App de traducción en tiempo real", f3: "Word Blaster: Juego de aprendizaje de vocabulario inglés", f4: "Otros incluyen práctica de debate y temporizadores de trabajo" }, cta: "Ver aplicaciones" } },
        about: { title: "Presentación del representante", motto: "La vida siempre se puede cambiar", subtitle: "La determinación de hacer lo que quieres ahora, no en la vejez", story: { p1: "Amaba Japón. Los paisajes urbanos atmosféricos, aguas termales que sanan mente y cuerpo, cocina local y gente cálida. Algún día, viajar por este querido Japón era mi sueño.", p2: "Sin embargo, la realidad era la de un asalariado ocupado. Las vacaciones prolongadas eran un sueño imposible, y me decía a mí mismo que 'recorrer Japón sería un placer para la jubilación' y casi me rendía.", p3: "Entonces llegó un punto de inflexión que cambiaría mi vida 180 grados. Fue mi infección con el nuevo coronavirus.", p4: "Solo, aislado en una habitación durante una semana. En una situación enfrentando el miedo a la muerte, solo un pensamiento cruzó mi mente: 'Si quiero hacer algo, debo hacerlo ahora o me arrepentiré. La vida es demasiado corta.'", p5: "El día que pude salir de mi habitación, mi segunda vida comenzó. En solo una semana de aislamiento, había terminado de planificar todo lo necesario para construir mi propia autocaravana.", p6: "Selección del vehículo base, diseño de cama deslizante para familia de cuatro, disposición de escritorio para trabajo, identificación de requisitos de registro número 8 para cumplir regulaciones, construcción de sistema de batería auxiliar, e incluso suministro de madera para el interior. El plano en mi cabeza estaba completamente terminado.", p7: "Inmediatamente compré un HiAce Super Long 2008 (¡mismo tamaño que una ambulancia!) por 1.7 millones de yenes. Desde allí, durante dos meses, dediqué todo el tiempo disponible al DIY - después del trabajo en días de semana, fines de semana y descansos durante trabajo remoto.", p8: "Y luego llegaron las vacaciones de verano que había estado apuntando. Antes del primer viaje de camping con mi familia, la autocaravana original única en el mundo finalmente se completó.", p9: "Ese viaje fue increíblemente conmovedor. Pasando hoteles llenos, experimentando la emoción del Festival Nebuta de Aomori y el Festival de Fuegos Artificiales de Nagaoka en primera línea. En el Hokkaido de verano, disfrutamos la naturaleza durante 10 días sin preocuparnos por los costos de alojamiento que se disparaban, rodeados por las sonrisas radiantes de nuestros hijos.", p10: "A través de esta experiencia, me convencí: '¿Puede un vehículo enriquecer la vida hasta este punto?'" }, additionalStory: { p11: "Y este viaje libre me dio el siguiente sueño, y aquel en el que apostaría mi vida. Es hacer de esta autocaravana una 'base de desarrollo para viajar por el mundo', creando nuevos valores mientras interactúo con culturas y personas de todo el mundo, sin estar atado por la ubicación.", p12: "Específicamente, desarrollar aplicaciones innovadoras usando tecnología IA y entregarlas a usuarios de todo el mundo a través de la plataforma Apple. Vertiendo inspiración obtenida de viajes en desarrollo en tiempo real, haciendo la vida de las personas más conveniente y rica. Encontré esta forma de contribución.", p13: "La bandera para realizar esta visión grandiosa es 'Trail❌fusion❌AI'.", p14: "Trail (Sendero): El 'sendero' físico de viajar por el mundo con una autocaravana, y el 'sendero' de vida que lleva a un estilo de vida que nadie ha recorrido jamás.", p15: "Fusion: La 'fusión' de la experiencia real que es el viaje con la creación digital que es el desarrollo de aplicaciones IA. Y 'fusionar' los pensamientos de usuarios de todo el mundo con los nuestros.", p16: "AI: El núcleo de nuestro negocio que abre nuestro futuro y crea posibilidades infinitas.", p17: "Sueños que casi había abandonado como asalariado. Pero en esta vida única, no quiero tener más arrepentimientos. Viajar por el mundo, crear valor y disfrutar plenamente los encuentros. Y algún día, cuando enfrente el final de mi vida, quiero sonreír desde el corazón diciendo 'Fue la mejor vida'. 'TrailfusionAI' es el nombre de ese juramento.", finalCta: "Ahora, ¿no se unirán a nosotros en una aventura llamada vida sin arrepentimientos?" }, callToAction: "Ahora, ¿no se unirán a nosotros en una aventura llamada vida sin arrepentimientos?", followMe: "Sígueme", interests: "Pasatiempos e Intereses", tags: ["Kendo", "Autocaravana", "Fabricación DIY", "Tour por Japón", "Gastronomía", "Aguas Termales", "Desarrollo de Apps", "Maratón", "Montañismo"] },
        mobileNav: { home: "Inicio", camping: "Fabricación de Autocaravana", apps: "Apps IA", contact: "Contacto" },
        contact: { title: "Contacto", subtitle: "Si tienes alguna pregunta o consulta, no dudes en contactarnos.", form: { name: "Nombre", namePlaceholder: "Juan Pérez", email: "Dirección de correo electrónico", emailPlaceholder: "example@mail.com", subject: "Asunto", subjectPlaceholder: "Asunto de tu consulta", message: "Mensaje", messagePlaceholder: "Por favor escribe tus preguntas o contenido de consulta", charCount: "caracteres", submit: "Enviar" }, info: { title: "Información de contacto", email: "Dirección de correo electrónico", youtube: "Canal TrailFusion AI" }, messages: { success: "Hemos recibido tu consulta. Gracias.", error: "Ocurrió un error. Por favor inténtalo de nuevo.", sending: "Enviando...", charCount: "caracteres" } },
        form: { sending: "Enviando...", success: "Hemos recibido tu consulta. Gracias.", error: "Ocurrió un error. Por favor inténtalo de nuevo.", charCount: "caracteres" },
        footer: { description: "Expande tus posibilidades con aplicaciones IA innovadoras y guías de construcción de autocaravana DIY.", camping: { title: "Autocaravana", items: { videos: "Proceso completo de fabricación DIY publicado en videos", gallery: "Galería de ejemplos completados", guide: "Guía de construcción de autocaravana DIY", requirements: "Requisitos estructurales de autocaravana" } }, apps: "Apps IA", contact: "Contacto", info: { title: "Información de contacto", email: "Dirección de correo electrónico", youtube: "YouTube", tiktok: "TikTok", twitter: "X (anteriormente Twitter)" } }
    }
    
    // All 10 languages now have complete translations for every section
};

// Export for use in i18n.js
if (typeof module !== 'undefined' && module.exports) {
    module.exports = completeTranslations;
}